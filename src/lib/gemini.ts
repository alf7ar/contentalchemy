...
const SYSTEM_PROMPTS = {
  instagram: `أنت خبير في إنشاء محتوى إنستغرام باللغة العربية. 
قم بإنشاء منشور إنستغرام جذاب بالعربية適合 للسوق المصري.
...
export async function generateContent(options: GenerateOptions): Promise<GenerateResult> {
  ...
  const prompt = `${SYSTEM_PROMPTS[options.platform]}
